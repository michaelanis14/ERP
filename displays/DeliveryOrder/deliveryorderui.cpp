...
DeliveryOrderUI::DeliveryOrderUI(QWidget *parent) :ERPDisplay(parent)
{
	...
	QObject::connect(deliveryorderserial, SIGNAL(currentIndexChanged(QString)), this, SLOT(selectionChanged(QString)));
...
void DeliveryOrderUI::fill(DeliveryOrder* deliveryorder){
	...
	serial->setText(QString::number(deliveryorder->Serial));
...
void DeliveryOrderUI::selectDeliveryOrder(){
	if(DeliveryOrder::GetStringList().contains(QString::number(this->deliveryorder->Serial)))
	{
		DeliveryOrder* con = DeliveryOrder::Get(QString::number(this->deliveryorder->Serial));
...
bool DeliveryOrderUI::save(){
	...
	if(serial->text().trimmed().isEmpty()){
		...
		serial->style()->polish(serial);
		serial->update();
		deliveryorder->Serial = serial->text().trimmed().toInt();
	}
...
void DeliveryOrderUI::selectionChanged(QString){
	project->addItems(Project::GetPairList(Project::QuerySelect("ContactID = "+QString::number(contact->getKey()))));
	QList<DeliveryOrder*> d = DeliveryOrder::QuerySelect("Serial = (SELECT MAX(Serial) from DeliveryOrder)");
	int i = 1;
	if(!d.isEmpty())
		i = d.first()->Serial +1;
