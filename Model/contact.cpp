...
Contact::Contact():  QSqlRelationalTableModel()
...
Qt::ItemFlags Contact::flags(const QModelIndex &index) const
{
	...
	if (index.column() == 2 || index.column() == 10)
		flags |= Qt::ItemIsEditable;
	//else flags |o Qt::ItemIsEditable;
	return flags;
}
//! [0]
...
bool Contact::setData(const QModelIndex &index, const QVariant &value, int /* role */)
{
	...
	//	qDebug() << (data(QSqlRelationalTableModel::index(index.row(), index.column())).toString() != value.toString().toLower());
	//	qDebug() << data(QSqlRelationalTableModel::index(index.row(), index.column())).toString()  << value.toString().toLower() ;
...
bool Contact::setLastName(int personId, const QString &lastName)
{
	qDebug() << "kk";
	QSqlQuery query;
